...
class Stewart_Platform_Linear(object):
    """
    Stewart Platform with 6 linear actuators (prismatic joints).

    Parameters
    ----------
    r_B        : radius of base circle (mm)
    r_P        : radius of platform circle (mm)
    gamma_B    : half-angle between anchor pairs on the base (rad)
    gamma_P    : half-angle between anchor pairs on the platform (rad)
    home_z     : resting height of the platform above the base (mm)
    ref_rotation: reference rotation around Z axis (rad), default 0
    """

    def __init__(s, r_B, r_P, gamma_B, gamma_P, home_z, ref_rotation=0):
        pi = np.pi
        ...
        psi_B = np.array([
            ...
            2*pi/3 + gamma_B,
            2*pi/3 + 2*pi/3 - gamma_B,
            2*pi/3 + 2*pi/3 + gamma_B]) + ref_rotation
        ...
        psi_P = np.array([
            ...
            pi/3 + 2*pi/3 - gamma_P,
            pi/3 + 2*pi/3 + gamma_P,
            pi/3 + 2*pi/3 + 2*pi/3 - gamma_P]) + ref_rotation
        ...
        s.gamma_B = gamma_B
        s.gamma_P = gamma_P
        s.home_pos = np.array([0.0, 0.0, home_z])
        s.B = B
        s.P = P
    ...
    def calculate(s, trans, rotation):
        """
        Solve inverse kinematics for linear actuators.

        Parameters
        ----------
        trans    : [tx, ty, tz]  translation of the platform (mm)
        rotation : [rx, ry, rz]  rotation angles Pitch/Roll/Yaw (rad)

        Returns
        -------
        lll : array of 6 leg lengths (mm)
        """
        trans    = np.asarray(trans,    dtype=float)
        rotation = np.asarray(rotation, dtype=float)

        R = np.matmul(
            np.matmul(s.rotZ(rotation[2]), s.rotY(rotation[1])),
            s.rotX(rotation[0]))

        s.l = (np.repeat(trans[:, np.newaxis],      6, axis=1)
